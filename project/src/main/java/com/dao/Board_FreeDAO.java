...
public class Board_FreeDAO {
	...
	public List<Board_FreeVO> getArticles(int start,int end){//수정 필요 모르겠음,,
		Connection conn = null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try {
			conn= DBcon.getConnection();
			pstmt=conn.prepareStatement("select * from ACI_BOARD_FREE order by fNum desc LIMIT ?, 5");
