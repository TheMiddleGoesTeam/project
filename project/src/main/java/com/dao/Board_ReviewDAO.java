...
public class Board_ReviewDAO {
    ...
    public List<Board_ReviewVO> getArticles(int start,int end){
        Connection conn = null;
        PreparedStatement pstmt=null;
        ResultSet rs=null;
        ...
        try {
            conn= DBcon.getConnection();
            String sql="select * from ACI_BOARD_REVIEW order by rNum desc LIMIT ?, 10";
 ...
 public Board_ReviewVO updateGetArticle(int num) {
     ...
     public int updateArticle(Board_ReviewVO vo) {
         ...
         try {
               ...
               rs= pstmt.executeQuery();
               if(rs.next()) {
                   drpasswd=rs.getString("rPass");
                   if(drpasswd.equals(vo.getrPass())) {
 ...
 public int deleteArticle(int num, String pass) {
    ...
    */
    ...
    public List<Board_ReviewVO> getArticles(String what, String content,int start,int end){
        Connection con = null;
        PreparedStatement pstmt=null;
        ResultSet rs=null;
        ...
        try {
            ...
            String sql="SELECT * FROM ACI_BOARD_REVIEW WHERE " + what + " LIKE ? "
                       + "ORDER BY rRef DESC, rStep ASC LIMIT ? OFFSET ?";
            pstmt=con.prepareStatement(sql);
            ...
            if(rs.next()) {
                articleList = new ArrayList<Board_ReviewVO>();
                do {
                    Board_ReviewVO article = new Board_ReviewVO();
                    article.setrNum(rs.getInt("rNum"));
                    article.setrWriter(rs.getString("rWriter"));
                    article.setrTitle(rs.getString("rTitle"));
                    article.setrPass(rs.getString("rPass"));
