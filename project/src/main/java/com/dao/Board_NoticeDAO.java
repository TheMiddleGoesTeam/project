...
public class Board_NoticeDAO {
    ...
    public List<Board_NoticeVO> getArticles(int start,int end){
        Connection conn = null;
        PreparedStatement pstmt=null;
        ResultSet rs=null;
 ...
 public Board_NoticeVO updateGetArticle(int num) {
     ...
     public int updateArticle(Board_NoticeVO vo) {
         ...
         try {
               con=DBcon.getConnection();
               sql1="select nPass from ACI_BOARD_NOTICE where nNum=?";
               pstmt = con.prepareStatement(sql1);
 ...
 public int deleteArticle(int num, String pass) {
    ...
    */
    ...
    public List<Board_NoticeVO> getArticles(String what, String content,int start,int end){
        Connection con = null;
        PreparedStatement pstmt=null;
        ResultSet rs=null;
        ...
        try {
            ...
            String sql="SELECT * FROM ACI_BOARD_NOTICE WHERE " + what + " LIKE ? "
                       + "ORDER BY nRef DESC, nStep ASC LIMIT ? OFFSET ?";
            pstmt=con.prepareStatement(sql);
            ...
            if(rs.next()) {
                articleList = new ArrayList<Board_NoticeVO>();
                do {
                    Board_NoticeVO article = new Board_NoticeVO();
                    article.setnNum(rs.getInt("nNum"));
                    article.setnWriter(rs.getString("nWriter"));
                    article.setnTitle(rs.getString("nTitle"));
                    article.setnPass(rs.getString("nPass"));
